...
if ($accion == 'agregar_producto_pasarela') {
    ...
    $query = "SELECT nombre1, precioUltimo FROM vista_almacen_stock WHERE idProducto = $idProducto LIMIT 1";
    $select = mysqli_query($conexion, $query);
    $fila = mysqli_fetch_array($select);
    $precioUltimo = $fila['precioUltimo'];
    $nombre = $fila['nombre1'];
    if (!$precioUltimo) {
        $query = "SELECT nombre1 FROM productos WHERE idProducto = $idProducto";
        $select = mysqli_query($conexion, $query);
        $fila = mysqli_fetch_array($select);
        $nombre = $fila['nombre1'];
        $precioUltimo = 0;
    }
    $query = "INSERT INTO pasarela_compra (
        idProducto,
        nombre,
        cantidad,
        precio,
...
if ($accion == 'modificar_producto_pasarela') {
    ...
    $query = "UPDATE pasarela_compra SET cantidad = $cantidad, precio = $precio, descuentoPorcentaje = $descuentoPorcentaje, descuentoValor = $descuentoValor, lote = '$lote', vencimiento = '$vencimiento' WHERE idProducto = $idProducto";
    $update = mysqli_query($conexion, $query);
}

if ($accion == 'quitar_producto_pasarela') {
    $idProducto = $_POST['idProducto'];
    $query = "DELETE FROM pasarela_compra WHERE idProducto = $idProducto";
    $update = mysqli_query($conexion, $query);
}
...
<div class="col">
    <table class="table table-sm table-striped" id="datatable">
        ...
        <tbody>
            <?php
            $i = 1;
            $query_pasarela = "SELECT * FROM pasarela_compra ORDER BY nombre";
            ...
            <tr>
                ...
                <td>
                    <input class="form-control form-control-sm txt_precio" type="number" value="<?=$precio?>">
                </td>
                <td>
                ...
                <td>
                    <a class="btn btn-warning btn-sm btn-modificar">
        ...
        <tfoot>
            <tr>
                ...
                <td colspan="8" class="table-dark fw-bold">Total: </td>
...
$('.btn-modificar').on('click', function() {
    ...
    if(descuentoValor > (cantidad*precio)*(1 - descuentoPorcentaje/100)) {
    ...
    $.ajax({
        type: "POST",
        url: "controllers/compras_pasarela.php",
        data: data,
